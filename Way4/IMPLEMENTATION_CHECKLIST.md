...
- P0 主骨架：`[~]`。P0-002～P0-025 已有代码、测试或运行证据，包含 effective geometry、cardinality lifecycle、INITIALIZED/PRESENT_UNOBSERVED、统一 action mask、adaptive scan、事件总线和时间模型；剩余主要是 P0-000 的项目级 freeze commit 与 P0-026 的独立 SMDP learner 效果证据。
...
- P0 Gate：多个 gate 已完成或有强证据；freeze commit、端到端大样本比较及若干理论/训练 gate 仍保持 `[~]`，不把局部测试等同于最终研究完成。

## P0 — deterministic hard skeleton

| 状态 | ID | 证据与结论 |
|---|---|---|
| [~] | P0-000 | `FREEZE_MANIFEST.md` 明确权威源码根/环境/入口；新增并执行 `scripts/freeze_fingerprint.py`，生成逐文件 SHA-256 与 tree fingerprint（`results/way4_source_fingerprint.json`），记录 Git HEAD `f0084a4` 与工作树未提交状态。仍缺最终 Git freeze commit，故保持 partial。 |
