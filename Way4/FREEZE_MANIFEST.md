...
- 当前 Git HEAD 为 `f0084a4`，但工作树仍有未提交变更；`scripts/freeze_fingerprint.py` 生成的 `results/way4_source_fingerprint.json` 提供当前 `src/`、`tests/`、`DESIGN.md` 和本 manifest 的逐文件 SHA-256 与 tree fingerprint。该 fingerprint 是可复现快照证据，不等同于 Git freeze commit。
